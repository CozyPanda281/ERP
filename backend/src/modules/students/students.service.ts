...
export class StudentsService {
  ...
  private decryptRow<T extends Record<string, any>>(
    row: T,
    fields: (keyof T)[],
  ): T {
    if (!row) return row;
    for (const field of fields) {
      if (row[field]) {
        try {
          (row as any)[field] = this.crypto.decrypt(row[field]);
        } catch {
  ...
  async createEnquiry(params: {
    ...
    const [inserted] = await this.db.db
      ...
      .values({
        ...
        parentPhone: this.encrypt(params.parentPhone),
        parentEmail: this.encrypt(params.parentEmail),
    ...
    return this.decryptRow(
      await this.findEnquiryById(inserted.id, params.branchId),
  ...
  async findEnquiriesByBranch(
    ...
    if (query.search)
      conditions.push(ilike(schema.enquiries.studentName, `%${query.search}%`));
    if (query.fromDate)
      conditions.push(
        sql`${schema.enquiries.createdAt} >= ${query.fromDate}::date`,
      );
    if (query.toDate)
      conditions.push(
        sql`${schema.enquiries.createdAt} <= ${query.toDate}::date`,
      );
  ...
  async convertEnquiryToApplication(id: string) {
    ...
    if (enquiry.convertedToApplication)
      throw new ConflictException('Enquiry already converted');

    const appNo = await this.generateApplicationNumber(
      enquiry.tenantId,
      enquiry.branchId,
    );
    ...
    if (enquiry.academicYearId)
      insertData.academicYearId = enquiry.academicYearId;
    const [insertedApp] = await this.db.db
    ...
    await this.db.db
      .update(schema.enquiries)
      .set({ convertedToApplication: true, updatedAt: new Date() })
      .where(eq(schema.enquiries.id, id));
    ...
    const [result] = await this.db.db
      ...
      .where(
        and(
          ...
          eq(schema.applications.branchId, branchId),
        ),
      );
  ...
  async createApplication(params: {
    ...
    if (encrypted.fatherPhone)
      encrypted.fatherPhone = this.encrypt(encrypted.fatherPhone);
    if (encrypted.fatherEmail)
      encrypted.fatherEmail = this.encrypt(encrypted.fatherEmail);
    if (encrypted.motherPhone)
      encrypted.motherPhone = this.encrypt(encrypted.motherPhone);
    if (encrypted.motherEmail)
      encrypted.motherEmail = this.encrypt(encrypted.motherEmail);
    if (encrypted.guardianPhone)
      encrypted.guardianPhone = this.encrypt(encrypted.guardianPhone);
    const [inserted] = await this.db.db
      .insert(schema.applications)
      .values(encrypted as any)
      .returning({ id: schema.applications.id });
    return this.decryptRow(
      await this.findApplicationById(inserted.id, params.branchId),
      [
        'phone',
        'email',
        'fatherPhone',
  ...
  async findApplicationsByBranch(
    branchId: string,
    query: {
      page?: number;
      limit?: number;
    ...
    if (query.status)
      conditions.push(eq(schema.applications.status, query.status));
    if (query.classId)
      conditions.push(eq(schema.applications.classId, query.classId));
    if (query.fromDate)
      conditions.push(
        sql`${schema.applications.createdAt} >= ${query.fromDate}::date`,
      );
    if (query.toDate)
      conditions.push(
        sql`${schema.applications.createdAt} <= ${query.toDate}::date`,
      );
  ...
  async admitApplication(id: string) {
    ...
    if (app.admitted)
      throw new ConflictException('Application already admitted');
    ...
    await this.db.db.transaction(async (tx) => {
      const [studentInserted] = await tx
  ...
  async createStudent(params: {
    ...
    const [inserted] = await this.db.db
      ...
      .values({
        ...
        email: encEmail,
        admissionDate:
          params.admissionDate || new Date().toISOString().split('T')[0],
      })
      .returning({ id: schema.students.id });
  ...
  async withdrawStudent(
    ...
    if (student.status === 'left')
      throw new BadRequestException('Student already withdrawn');
    await this.db.db
      .update(schema.students)
      .set({
        status: 'left',
  ...
  async getStudentParents(studentId: string) {
    ...
    return this.db.db
      .select({
        ...
        relationship: schema.parents.relationship,
        phone: schema.parents.phone,
        email: schema.parents.email,
        occupation: schema.parents.occupation,
        isPrimary: schema.parents.isPrimary,
        studentRelationship: schema.studentParents.relationship,
        isEmergencyContact: schema.studentParents.isEmergencyContact,
      })
      .from(schema.parents)
      .innerJoin(
        schema.studentParents,
        eq(schema.studentParents.parentId, schema.parents.id),
      )
  ...
  async findDocumentById(id: string, tenantId?: string) {
    ...
    if (tenantId)
      conditions.push(eq(schema.studentDocuments.tenantId, tenantId));
  ...
  async updateDocument(id: string, params: any) {
    ...
    await this.db.db
      ...
      .set(allowed)
      .where(eq(schema.studentDocuments.id, id));
  ...
  async createAcademicRecord(params: {
    ...
    const [inserted] = await this.db.db
      .insert(schema.studentAcademicRecords)
      .values(params)
      .returning({ id: schema.studentAcademicRecords.id });
    return this.findAcademicRecordById(
      inserted.id,
      params.tenantId,
      params.branchId,
    );
  ...
  async findAcademicRecordById(
    ...
    if (tenantId)
      conditions.push(eq(schema.studentAcademicRecords.tenantId, tenantId));
    if (branchId)
      conditions.push(eq(schema.studentAcademicRecords.branchId, branchId));
