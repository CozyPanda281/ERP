...
import { eq, and, isNull, or, desc, asc, count, sql, inArray, gte, lte } from 'drizzle-orm';
...
export class AttendanceService {
  ...
  private async validateTimetableEntry(timetableEntryId: string, branchId: string) {
    const [entry] = await this.db.db.select({
    ...
    if (!entry) throw new NotFoundException('Timetable entry not found in this branch');
    return entry;
  }

  private async validateStudents(studentIds: string[], tenantId: string, branchId: string) {
    if (!studentIds.length) throw new BadRequestException('At least one student is required');

    const existing = await this.db.db.select({ id: schema.students.id })
  ...
  async createSession(params: {
    ...
    timetableEntryId: string; date: string;
    records: Array<{ studentId: string; status: string; remarks?: string }>;
  }) {
    const entry = await this.validateTimetableEntry(params.timetableEntryId, params.branchId);
    ...
    const existingSession = await this.db.db.select({ id: schema.attendance.id })
    ...
    if (existingSession.length) {
      throw new ConflictException('Attendance already marked for this timetable entry on this date');
    }

    let sessionId = '';

    await this.db.db.transaction(async (tx) => {
      const [session] = await tx.insert(schema.attendance).values({
      ...
      await tx.update(schema.attendance).set({
  ...
  async getSessionById(id: string, branchId: string) {
    const [session] = await this.db.db.select()
      .from(schema.attendance)
      .where(and(eq(schema.attendance.id, id), eq(schema.attendance.branchId, branchId)))
      .limit(1);
  ...
  async updateRecord(recordId: string, branchId: string, params: { status?: string; remarks?: string }) {
    const [record] = await this.db.db.select({
      id: schema.attendanceRecords.id,
    ...
    await this.db.db.update(schema.attendanceRecords)
      .set(updateData)
      .where(eq(schema.attendanceRecords.id, recordId));

    await this.recalculateSession(record.attendanceId);

    return this.db.db.select().from(schema.attendanceRecords).where(eq(schema.attendanceRecords.id, recordId)).limit(1).then(r => r[0]);
  }

  private async recalculateSession(attendanceId: string) {
    const [counts] = await this.db.db.select({
      present: sql`COUNT(*) FILTER (WHERE ${schema.attendanceRecords.status} = 'present')`.as<number>(),
      absent: sql`COUNT(*) FILTER (WHERE ${schema.attendanceRecords.status} = 'absent')`.as<number>(),
    }).from(schema.attendanceRecords)
      .where(eq(schema.attendanceRecords.attendanceId, attendanceId));

    await this.db.db.update(schema.attendance).set({
  ...
  async findByBranch(branchId: string, query: {
    page?: number; limit?: number; classId?: string; sectionId?: string;
    ...
    if (query.sectionId) conditions.push(eq(schema.attendance.sectionId, query.sectionId));
    if (query.fromDate) conditions.push(gte(schema.attendance.date, query.fromDate));
    if (query.toDate) conditions.push(lte(schema.attendance.date, query.toDate));
    ...
    const data = await this.db.db.select({
      ...
      .leftJoin(schema.classes, eq(schema.classes.id, schema.attendance.classId))
      .leftJoin(schema.subjects, eq(schema.subjects.id, schema.attendance.subjectId))
      .where(and(...conditions))
      .orderBy(desc(schema.attendance.date))
      .limit(limit).offset(offset);

    const [total] = await this.db.db.select({ count: count() })
      .from(schema.attendance).where(and(...conditions));
  ...
  async findByStudent(studentId: string, branchId: string, query: { fromDate?: string; toDate?: string; page?: number; limit?: number }) {
    const page = query.page || 1;
  ...
  async getSummary(studentId: string, branchId: string, query: { fromDate?: string; toDate?: string }) {
    const conditions: any[] = [
      eq(schema.attendanceRecords.studentId, studentId),
      eq(schema.attendanceRecords.branchId, branchId),
    ];
    ...
    const counts = await this.db.db.select({
      ...
      excused: sql`COUNT(*) FILTER (WHERE ${schema.attendanceRecords.status} = 'excused')`.as<number>(),
    }).from(schema.attendanceRecords)
      .innerJoin(schema.attendance, eq(schema.attendance.id, schema.attendanceRecords.attendanceId))
      .where(and(...conditions))
      .then(r => r[0]);
    ...
    return {
      ...
      percentage: total > 0 ? Math.round((Number(counts.present) + Number(counts.late)) / total * 100) : 0,
    };
  }
  ...
  async getDailyReport(branchId: string, date: string) {
    ...
    const sessions = await this.db.db.select({
      id: schema.attendance.id,
      ...
      .leftJoin(schema.classes, eq(schema.classes.id, schema.attendance.classId))
      .leftJoin(schema.subjects, eq(schema.subjects.id, schema.attendance.subjectId))
      .where(and(eq(schema.attendance.branchId, branchId), eq(schema.attendance.date, date)))
