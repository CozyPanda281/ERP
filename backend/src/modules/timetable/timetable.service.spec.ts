...
describe('TimetableService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [TimetableService, { provide: DatabaseProvider, useValue: mockDb }],
    }).compile();
  ...
  describe('create', () => {
    it('should create a timetable', async () => {
      ...
      const result = await service.create({ tenantId: 't-1', branchId: 'b-1', name: 'Spring 2026', classId: 'c-1' });
      expect(result.name).toBe('Spring 2026');
    });

    it('should reject duplicate name', async () => {
      mockDb.setDrizzleResults([{ id: 'tt-1' }]);
      await expect(service.create({ tenantId: 't-1', branchId: 'b-1', name: 'Duplicate', classId: 'c-1' })).rejects.toThrow(ConflictException);
    });
  });
  ...
  describe('addEntry', () => {
    it('should add entry with overlap check', async () => {
      ...
      const result = await service.addEntry('tt-1', { dayOfWeek: 0, subjectId: 'sub-1', startTime: '08:00', endTime: '08:45' });
    ...
    it('should reject overlapping time', async () => {
      ...
      await expect(service.addEntry('tt-1', { dayOfWeek: 0, subjectId: 'sub-1', startTime: '08:00', endTime: '09:00' })).rejects.toThrow(ConflictException);
    });
  });
  ...
  describe('removeEntry', () => {
    it('should throw on missing entry', async () => {
      mockDb.setDrizzleResults([]);
      await expect(service.removeEntry('bad')).rejects.toThrow(NotFoundException);
    });
  });
});
