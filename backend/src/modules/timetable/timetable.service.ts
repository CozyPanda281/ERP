...
export class TimetableService {
  ...
  async create(params: {
    ...
    const [existing] = await this.db.db
      .select({ id: schema.timetables.id })
      .from(schema.timetables)
      .where(
        and(
          eq(schema.timetables.branchId, params.branchId),
          eq(schema.timetables.name, params.name),
        ),
      )
      .limit(1);
    if (existing)
      throw new ConflictException('Timetable with this name already exists');
  ...
  async findById(id: string, branchId?: string) {
    ...
    const entries = await this.db.db
      .select({
        ...
        teacherId: schema.timetableEntries.teacherId,
      ...
      .orderBy(
        schema.timetableEntries.dayOfWeek,
        schema.timetableEntries.startTime,
      );
  ...
  async findByBranch(
    ...
    if (query.classId)
      conditions.push(eq(schema.timetables.classId, query.classId));
    if (query.isActive !== undefined)
      conditions.push(
        eq(schema.timetables.isActive, query.isActive === 'true'),
      );
  ...
  async setActive(id: string) {
    ...
    await this.db.db
      ...
      .where(
        and(
          eq(schema.timetables.branchId, timetable.branchId),
          eq(schema.timetables.classId, timetable.classId),
        ),
      );
  ...
  private async checkOverlap(
    ...
    if (excludeEntryId)
      conditions.push(sql`${schema.timetableEntries.id} != ${excludeEntryId}`);
    ...
    if (overlapping.length)
      throw new ConflictException('Time slot overlaps with an existing entry');
  }
  ...
  async addEntry(
    ...
    const [timetable] = await this.db.db
      .select({
        id: schema.timetables.id,
        tenantId: schema.timetables.tenantId,
      })
      .from(schema.timetables)
      .where(eq(schema.timetables.id, timetableId))
      .limit(1);
  ...
  async updateEntry(id: string, params: any) {
    const [existing] = await this.db.db
      .select()
      .from(schema.timetableEntries)
      .where(eq(schema.timetableEntries.id, id))
      .limit(1);
  ...
  async removeEntry(id: string) {
    ...
    if (!existing) throw new NotFoundException('Timetable entry not found');
    await this.db.db
      .delete(schema.timetableEntries)
      .where(eq(schema.timetableEntries.id, id));
  }
  ...
  async findByTeacher(teacherId: string, branchId: string) {
    const entries = await this.db.db
      .select({
        id: schema.timetableEntries.id,
        timetableId: schema.timetableEntries.timetableId,
        dayOfWeek: schema.timetableEntries.dayOfWeek,
        startTime: schema.timetableEntries.startTime,
        endTime: schema.timetableEntries.endTime,
        roomNumber: schema.timetableEntries.roomNumber,
        timetableName: schema.timetables.name,
      ...
      .where(
        and(
          eq(schema.timetableEntries.teacherId, teacherId),
