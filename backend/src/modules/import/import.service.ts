...
export class ImportService {
  ...
  async upload(
    ...
    const [batch] = await this.db.db
      ...
      .values({
        ...
        validationErrors: validatedRows
          .filter((r: any) => !r.valid)
          .map((r: any) => ({ row: r.rowNumber, errors: r.errors })) as any,
        createdBy: userId,
      })
  ...
  async listBatches(
    ...
    if (entityType)
      conditions.push(eq(schema.importBatches.entityType, entityType));
  ...
  async updatePreview(
    ...
    if (batch.status !== 'pending_review')
      throw new BadRequestException('Batch is not in review state');

    const preview = (batch.previewData as any[]) || [];
    for (const update of rows) {
      const existing = preview.find(
        (p: any) => p.rowNumber === update.rowNumber,
      );
  ...
  async deploy(id: string, userId: string) {
    ...
    if (batch.status !== 'approved')
      throw new BadRequestException('Batch must be approved before deployment');

    const deployer = getDeployer(batch.entityType);
    const allRows: ParsedRow[] = ((batch.previewData as any[]) || [])
    ...
    this.logger.log(
      `Import batch ${id} deployed: ${result.inserted} inserted, ${result.failed} failed`,
    );
  ...
  async rollback(id: string, userId: string) {
    ...
    for (const row of rollbackData) {
      try {
        ...
        if (fn && ln) {
          const [toDelete] = await this.db.db
            .select({ id: schema.students.id })
            .from(schema.students)
            .where(
              and(
                eq(schema.students.tenantId, batch.tenantId),
          ...
          if (toDelete) {
            await this.db.db
              ...
              .where(eq(schema.students.id, toDelete.id));
            removed++;
          }
        }
      } catch {
    ...
    this.logger.warn(
      `Import batch ${id} rolled back by ${userId}: ${removed} records removed`,
    );

    return {
      message: `Rolled back ${removed} records`,
      batch: await this.getBatch(id),
    };
  }
}
