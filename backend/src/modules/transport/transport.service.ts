...
export class TransportService {
  ...
  async findVehiclesByBranch(
    ...
    const conditions = [
      eq(schema.transportVehicles.branchId, branchId),
      isNull(schema.transportVehicles.deletedAt),
    ];
    const data = await this.db.db
      .select()
      .from(schema.transportVehicles)
      .where(and(...conditions))
      .orderBy(desc(schema.transportVehicles.createdAt))
      .limit(limit)
      .offset(offset);
  ...
  async findVehicleById(id: string) {
    const [result] = await this.db.db
      ...
      .where(
        and(
          eq(schema.transportVehicles.id, id),
          isNull(schema.transportVehicles.deletedAt),
        ),
      )
      .limit(1);
  ...
  async updateVehicle(
    ...
    if (Object.keys(values).length > 1) {
      await this.db.db
        ...
        .set(values)
        .where(eq(schema.transportVehicles.id, id));
    }
  ...
  async findRoutesByBranch(
    branchId: string,
    query?: { page?: number; limit?: number; vehicleId?: string },
  ) {
    const page = query?.page || 1;
    const limit = Math.min(query?.limit || 20, 100);
    const offset = (page - 1) * limit;
    const conditions: any[] = [
      eq(schema.transportRoutes.branchId, branchId),
      isNull(schema.transportRoutes.deletedAt),
    ];
    if (query?.vehicleId)
      conditions.push(eq(schema.transportRoutes.vehicleId, query.vehicleId));
    const data = await this.db.db
      .select()
      .from(schema.transportRoutes)
      .where(and(...conditions))
      .orderBy(desc(schema.transportRoutes.createdAt))
      .limit(limit)
      .offset(offset);
    const [total] = await this.db.db
      .select({ count: count() })
  ...
  async findRouteById(id: string) {
    const [result] = await this.db.db
      ...
      .where(
        and(
          eq(schema.transportRoutes.id, id),
          isNull(schema.transportRoutes.deletedAt),
        ),
      )
      .limit(1);
  ...
  async assignStudent(params: {
    ...
    const [inserted] = await this.db.db
      .insert(schema.transportAssignments)
      .values({
        tenantId: params.tenantId,
        branchId: params.branchId,
    ...
    const [assignment] = await this.db.db
      ...
      .where(eq(schema.transportAssignments.id, inserted.id))
      .limit(1);
    return assignment;
  }
  ...
  async findAssignmentsByBranch(
    ...
    const offset = (page - 1) * limit;
    const conditions: any[] = [
      eq(schema.transportAssignments.branchId, branchId),
      isNull(schema.transportAssignments.deletedAt),
    ];
    if (query?.routeId)
      conditions.push(eq(schema.transportAssignments.routeId, query.routeId));
    if (query?.studentId)
      conditions.push(
        eq(schema.transportAssignments.studentId, query.studentId),
      );
    const data = await this.db.db
      .select()
      .from(schema.transportAssignments)
      .where(and(...conditions))
      .orderBy(desc(schema.transportAssignments.createdAt))
      .limit(limit)
      .offset(offset);
    const [total] = await this.db.db
      .select({ count: count() })
  ...
  async unassignStudent(id: string) {
    const [existing] = await this.db.db
      .select({ id: schema.transportAssignments.id })
      .from(schema.transportAssignments)
      .where(
        and(
          eq(schema.transportAssignments.id, id),
          isNull(schema.transportAssignments.deletedAt),
        ),
      )
      .limit(1);
    ...
    await this.db.db
      .update(schema.transportAssignments)
      .set({ deletedAt: new Date() })
