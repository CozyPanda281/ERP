...
describe('TransportService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [TransportService, { provide: DatabaseProvider, useValue: mockDb }],
    }).compile();
  ...
  describe('createVehicle', () => {
    it('should create a vehicle', async () => {
      mockDb.setDrizzleResults([{ id: 'v-1' }], [{ id: 'v-1', vehicleNumber: 'KA-01-1234', deletedAt: null }]);
      const result = await service.createVehicle({ ...T, vehicleNumber: 'KA-01-1234' });
  ...
  describe('findVehicleById', () => {
    it('should return a vehicle', async () => {
      mockDb.setDrizzleResults([{ id: 'v-1', vehicleNumber: 'KA-01-1234', deletedAt: null }]);
      const result = await service.findVehicleById('v-1');
      expect(result.id).toBe('v-1');
    });
    it('should throw on missing vehicle', async () => {
      mockDb.setDrizzleResults([]);
      await expect(service.findVehicleById('bad')).rejects.toThrow(NotFoundException);
    });
  });

  describe('createRoute', () => {
    it('should create a route', async () => {
      mockDb.setDrizzleResults([{ id: 'r-1' }], [{ id: 'r-1', name: 'Route A', deletedAt: null }]);
  ...
  describe('findRouteById', () => {
    it('should throw on missing route', async () => {
      mockDb.setDrizzleResults([]);
      await expect(service.findRouteById('bad')).rejects.toThrow(NotFoundException);
    });
  });

  describe('assignStudent', () => {
    it('should assign a student', async () => {
      mockDb.setDrizzleResults([{ id: 'as-1' }], [{ id: 'as-1', studentId: 's-1', routeId: 'r-1' }]);
      const result = await service.assignStudent({ ...T, studentId: 's-1', routeId: 'r-1', effectiveFrom: '2026-01-01' });
