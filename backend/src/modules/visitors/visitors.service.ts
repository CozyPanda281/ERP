...
export class VisitorsService {
  ...
  async checkIn(params: {
    ...
    const [inserted] = await this.db.db
      ...
      .values({ ...params, checkInTime: new Date() })
      .returning({ id: schema.visitors.id });
    const [visitor] = await this.db.db
      .select()
      .from(schema.visitors)
      .where(eq(schema.visitors.id, inserted.id))
      .limit(1);
    return visitor;
  }
  ...
  async findByBranch(
    ...
    if (query?.status)
      conditions.push(eq(schema.visitors.status, query.status));
    if (query?.fromDate)
      conditions.push(
        sql`${schema.visitors.checkInTime} >= ${query.fromDate}::timestamp`,
      );
    if (query?.toDate)
      conditions.push(
        sql`${schema.visitors.checkInTime} <= ${query.toDate}::timestamp`,
      );
  ...
  async findById(id: string) {
    const [result] = await this.db.db
      .select()
      .from(schema.visitors)
      .where(eq(schema.visitors.id, id))
      .limit(1);
  ...
  async checkOut(id: string) {
    ...
    if (visitor.status === 'checked_out')
      throw new NotFoundException('Visitor already checked out');
    await this.db.db
