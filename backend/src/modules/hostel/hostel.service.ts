...
export class HostelService {
  ...
  async createHostel(params: {
    ...
    const [inserted] = await this.db.db
      ...
      .values({
        ...
        totalRooms: params.totalRooms ?? 0,
        totalBeds: params.totalBeds ?? 0,
        notes: params.notes,
      })
  ...
  async findHostelsByBranch(
    ...
    const conditions = [
      eq(schema.hostels.branchId, branchId),
      isNull(schema.hostels.deletedAt),
    ];
    const data = await this.db.db
      .select()
      .from(schema.hostels)
      .where(and(...conditions))
      .orderBy(desc(schema.hostels.createdAt))
      .limit(limit)
      .offset(offset);
  ...
  async findHostelById(id: string) {
    const [result] = await this.db.db
      .select()
      .from(schema.hostels)
      .where(and(eq(schema.hostels.id, id), isNull(schema.hostels.deletedAt)))
      .limit(1);
  ...
  async createRoom(params: {
    ...
    const [inserted] = await this.db.db
      .insert(schema.hostelRooms)
      .values({
        hostelId: params.hostelId,
        roomNumber: params.roomNumber,
        floor: params.floor,
        capacity: params.capacity ?? 1,
        bedCount: params.bedCount ?? 1,
    ...
    const [room] = await this.db.db
      ...
      .where(eq(schema.hostelRooms.id, inserted.id))
      .limit(1);
    return room;
  }

  async findRoomsByHostel(
    hostelId: string,
    query?: { page?: number; limit?: number },
  ) {
    const page = query?.page || 1;
    ...
    const offset = (page - 1) * limit;
    const conditions = [
      eq(schema.hostelRooms.hostelId, hostelId),
      isNull(schema.hostelRooms.deletedAt),
    ];
    const data = await this.db.db
      .select()
      .from(schema.hostelRooms)
      .where(and(...conditions))
      .orderBy(schema.hostelRooms.roomNumber)
      .limit(limit)
      .offset(offset);
    const [total] = await this.db.db
      .select({ count: count() })
  ...
  async allocateBed(params: {
    ...
    const [inserted] = await this.db.db
      .insert(schema.hostelBedAllocations)
      .values({
        tenantId: params.tenantId,
        branchId: params.branchId,
    ...
    const [alloc] = await this.db.db
      .select()
      .from(schema.hostelBedAllocations)
      .where(eq(schema.hostelBedAllocations.id, inserted.id))
      .limit(1);
    return alloc;
  }

  async findAllocationsByBranch(
    branchId: string,
    query?: {
      page?: number;
      limit?: number;
    ...
    const conditions: any[] = [
      eq(schema.hostelBedAllocations.branchId, branchId),
      isNull(schema.hostelBedAllocations.deletedAt),
    ];
    if (query?.roomId)
      conditions.push(eq(schema.hostelBedAllocations.roomId, query.roomId));
    if (query?.studentId)
      conditions.push(
        eq(schema.hostelBedAllocations.studentId, query.studentId),
      );
    const data = await this.db.db
      .select()
      .from(schema.hostelBedAllocations)
  ...
  async vacateBed(id: string) {
    const [existing] = await this.db.db
      .select({ id: schema.hostelBedAllocations.id })
      .from(schema.hostelBedAllocations)
      .where(
        and(
          eq(schema.hostelBedAllocations.id, id),
          isNull(schema.hostelBedAllocations.deletedAt),
        ),
      )
      .limit(1);
    ...
    await this.db.db
      .update(schema.hostelBedAllocations)
      .set({
        vacateDate: new Date().toISOString().split('T')[0],
        status: 'vacated',
        updatedAt: new Date(),
      })
      .where(eq(schema.hostelBedAllocations.id, id));
    return { success: true };
  }
}
