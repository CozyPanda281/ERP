...
describe('HostelService', () => {
  ...
  describe('createHostel', () => {
    it('should create a hostel', async () => {
      mockDb.setDrizzleResults(
        [{ id: 'h-1' }],
        [{ id: 'h-1', name: 'Boys Hostel', deletedAt: null }],
      );
  ...
  describe('findHostelById', () => {
    ...
    it('should throw on missing hostel', async () => {
      mockDb.setDrizzleResults([]);
      await expect(service.findHostelById('bad')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('createRoom', () => {
    it('should create a room', async () => {
      mockDb.setDrizzleResults(
        [{ id: 'rm-1' }],
        [{ id: 'rm-1', roomNumber: '101', hostelId: 'h-1' }],
      );
  ...
  describe('allocateBed', () => {
    it('should allocate a bed', async () => {
      mockDb.setDrizzleResults(
        [{ id: 'al-1' }],
        [{ id: 'al-1', studentId: 's-1', roomId: 'rm-1' }],
      );
      const result = await service.allocateBed({
        ...T,
        roomId: 'rm-1',
        studentId: 's-1',
