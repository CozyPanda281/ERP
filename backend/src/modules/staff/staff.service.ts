...
export class StaffService {
  ...
  async findByBranch(
    ...
    if (query?.isTeaching !== undefined)
      conditions.push(eq(schema.staff.isTeaching, query.isTeaching));
    if (query?.search)
      conditions.push(
        sql`(${schema.staff.firstName}::text ILIKE ${'%' + query.search + '%'} OR ${schema.staff.lastName}::text ILIKE ${'%' + query.search + '%'} OR ${schema.staff.employeeCode}::text ILIKE ${'%' + query.search + '%'})`,
      );
  ...
  async findById(id: string) {
    const [result] = await this.db.db
      .select()
      .from(schema.staff)
      .where(and(eq(schema.staff.id, id), isNull(schema.staff.deletedAt)))
      .limit(1);
  ...
  async findDocumentsByStaff(staffId: string) {
    return this.db.db
      ...
      .orderBy(desc(schema.staffDocuments.createdAt));
  }
}
