...
import { NotificationQueryDto, AnnouncementQueryDto } from './dto/communication-query.dto';
...
export class CommunicationController {
  ...
  async updateTemplate(@Param('id') id: string, @Body() body: UpdateTemplateDto, @CurrentUser() user: any) {
  ...
  async createNotification(@Body() body: CreateNotificationDto, @CurrentUser() user: any) {
    return this.service.createNotification({
      ...body, tenantId: user.tenantId, branchId: user.branchId, senderId: user.id,
    });
  }

  @Get('notifications/mine')
  @Roles(ROLES.SUPER_ADMIN, ROLES.PRINCIPAL, ROLES.TEACHER, ROLES.STUDENT, ROLES.PARENT)
  @ApiOperation({ summary: 'Get my notifications' })
  async getMyNotifications(@CurrentUser() user: any, @Query() query: NotificationQueryDto) {
  ...
  async createAnnouncement(@Body() body: CreateAnnouncementDto, @CurrentUser() user: any) {
    return this.service.createAnnouncement({
      ...body, tenantId: user.tenantId, branchId: user.branchId, createdBy: user.id,
    });
  }
  ...
  async getAnnouncements(@CurrentUser() user: any, @Query() query: AnnouncementQueryDto) {
  ...
  async updateAnnouncement(@Param('id') id: string, @Body() body: UpdateAnnouncementDto, @CurrentUser() user: any) {
