...
export class TenantsService {
  ...
  async create(params: {
    ...
    const existing = await this.db.db.select()
      .from(schema.tenants)
      .where(and(eq(schema.tenants.slug, params.slug), isNull(schema.tenants.deletedAt)))
      .limit(1);
    ...
    const [ownerRole] = await this.db.db.select()
      .from(schema.roles)
      .where(and(eq(schema.roles.tenantId, tenantId), eq(schema.roles.slug, 'organization-owner')))
      .limit(1);
  ...
  async findAll(query: {
    ...
    const result = await this.db.db.select({
      ...
      .from(schema.tenants)
      .leftJoin(schema.subscriptions, and(eq(schema.subscriptions.tenantId, schema.tenants.id), inArray(schema.subscriptions.status, ['active', 'trial'])))
  ...
  async getTenantStats() {
    const [result] = await this.db.db.select({
      ...
      suspended: sql`COUNT(*) FILTER (WHERE status = 'suspended')`.mapWith(Number),
      newLast30Days: sql`COUNT(*) FILTER (WHERE created_at >= NOW() - INTERVAL '30 days')`.mapWith(Number),
    })
      .from(schema.tenants)
  ...
  async update(id: string, params: Partial<{ name: string; email: string; phone: string; address: string; city: string; state: string; country: string }>) {
    ...
    await this.db.db.update(schema.tenants)
  ...
  async updateStatus(id: string, status: string) {
    ...
    if (status === 'suspended') {
      await this.db.db.update(schema.subscriptions)
        .set({ status: 'cancelled', cancelledAt: new Date() })
        .where(and(eq(schema.subscriptions.tenantId, id), inArray(schema.subscriptions.status, ['active', 'trial'])));
    }
  ...
  async firstRunSetup(tenantId: string, params: {
    branchName: string;
    branchCode: string;
    ...
    if (existing.length) {
      throw new BadRequestException('Tenant already has branches. First-run setup not available.');
    }
