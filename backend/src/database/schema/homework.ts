import { pgTable, uuid, varchar, text, integer, boolean, jsonb, timestamp, numeric, index, uniqueIndex } from 'drizzle-orm/pg-core';
...
export const homework = pgTable('homework', {
  ...
  dueIdx: index('idx_homework_due').on(table.dueDate).where(sql`status = 'active'`),
}));
...
export const homeworkSubmissions = pgTable('homework_submissions', {
  ...
  homeworkStudentUnique: uniqueIndex('homework_submissions_homework_id_student_id_key').on(table.homeworkId, table.studentId),
  homeworkIdx: index('idx_homework_submissions_homework').on(table.homeworkId),
...
export const assignments = pgTable('assignments', {
  ...
  title: varchar('title', { length: 255 }).notNull(),
  description: text('description'),
  assignmentType: varchar('assignment_type', { length: 50 }).default('written'),
  attachmentUrls: jsonb('attachment_urls').default([]),
  dueDate: timestamp('due_date', { withTimezone: true }).notNull(),
  maxMarks: integer('max_marks'),
  status: varchar('status', { length: 50 }).default('active'),
...
export const assignmentSubmissions = pgTable('assignment_submissions', {
  ...
  assignmentStudentUnique: uniqueIndex('assignment_submissions_assignment_id_student_id_key').on(table.assignmentId, table.studentId),
  assignmentIdx: index('idx_assignment_submissions_assignment').on(table.assignmentId),
