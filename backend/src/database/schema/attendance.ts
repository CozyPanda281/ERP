...
export const attendance = pgTable(
  ...
  (table) => ({
    uniquePerTimetableEntryDate: uniqueIndex(
      'attendance_timetable_entry_date_key',
    ).on(table.timetableEntryId, table.date),
    uniquePerClassDate: uniqueIndex('attendance_class_date_key').on(
      table.tenantId,
      table.branchId,
      table.classId,
      table.sectionId,
    ...
    classDateIdx: index('idx_attendance_class_date').on(
      table.classId,
      table.date,
    ),
  }),
...
export const attendanceRecords = pgTable(
  ...
  (table) => ({
    attendanceStudentUnique: uniqueIndex(
      'attendance_records_attendance_id_student_id_key',
    ).on(table.attendanceId, table.studentId),
    attendanceIdx: index('idx_attendance_records_attendance').on(
      table.attendanceId,
    ),
    studentIdx: index('idx_attendance_records_student').on(table.studentId),
    branchDateIdx: index('idx_attendance_records_branch_date').on(
      table.branchId,
      table.attendanceId,
    ),
  }),
...
export const staffAttendance = pgTable(
  ...
  (table) => ({
    uniquePerStaffDay: uniqueIndex(
      'staff_attendance_tenant_id_branch_id_staff_id_date_key',
    ).on(table.tenantId, table.branchId, table.staffId, table.date),
