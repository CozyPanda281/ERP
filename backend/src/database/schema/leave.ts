import { pgTable, uuid, varchar, text, integer, boolean, timestamp, date, index, uniqueIndex } from 'drizzle-orm/pg-core';
...
export const leaveTypes = pgTable('leave_types', {
  ...
  tenantCodeUnique: uniqueIndex('leave_types_tenant_id_code_key').on(table.tenantId, table.code),
}));
...
export const leaveRequests = pgTable('leave_requests', {
  ...
  datesIdx: index('idx_leave_requests_dates').on(table.startDate, table.endDate),
}));

export const leaveTypesRelations = relations(leaveTypes, ({ one, many }) => ({
  tenant: one(tenants, { fields: [leaveTypes.tenantId], references: [tenants.id] }),
  leaveRequests: many(leaveRequests),
}));

export const leaveRequestsRelations = relations(leaveRequests, ({ one }) => ({
  tenant: one(tenants, { fields: [leaveRequests.tenantId], references: [tenants.id] }),
  branch: one(branches, { fields: [leaveRequests.branchId], references: [branches.id] }),
