...
export const feeConcessions = pgTable('fee_concessions', {
  ...
  feeStructureItemId: uuid('fee_structure_item_id').references(() => feeStructureItems.id),
...
export const studentFeeAccounts = pgTable('student_fee_accounts', {
  ...
  studentYearUnique: uniqueIndex('student_fee_accounts_student_id_academic_year_id_key').on(table.studentId, table.academicYearId),
  studentIdx: index('idx_fee_accounts_student').on(table.studentId),
  statusIdx: index('idx_fee_accounts_status').on(table.status),
  dueIdx: index('idx_fee_accounts_due').on(table.totalDue).where(sql`total_due > 0`),
}));
...
export const feeTransactions = pgTable('fee_transactions', {
  ...
  branchId: uuid('branch_id').notNull().references(() => branches.id, { onDelete: 'cascade' }),
  studentId: uuid('student_id').notNull().references(() => students.id, { onDelete: 'cascade' }),
  feeAccountId: uuid('fee_account_id').references(() => studentFeeAccounts.id),
  ...
  tenantBranchTxnNoUnique: uniqueIndex('fee_transactions_tenant_id_branch_id_transaction_no_key').on(table.tenantId, table.branchId, table.transactionNo),
...
export const feeReceipts = pgTable('fee_receipts', {
  id: uuid('id').defaultRandom().primaryKey(),
  tenantId: uuid('tenant_id').notNull().references(() => tenants.id, { onDelete: 'cascade' }),
  transactionId: uuid('transaction_id').notNull().references(() => feeTransactions.id, { onDelete: 'cascade' }),
  receiptNumber: varchar('receipt_number', { length: 50 }).notNull(),
  receiptDate: date('receipt_date').notNull(),
  receiptUrl: text('receipt_url'),
  createdAt: timestamp('created_at', { withTimezone: true }).defaultNow(),
}, (table) => ({
  tenantReceiptNoUnique: uniqueIndex('fee_receipts_tenant_id_receipt_number_key').on(table.tenantId, table.receiptNumber),
  transactionIdx: index('idx_fee_receipts_transaction').on(table.transactionId),
}));
...
export const feeInvoices = pgTable('fee_invoices', {
  ...
  tenantBranchInvoiceNoUnique: uniqueIndex('fee_invoices_tenant_id_branch_id_invoice_number_key').on(table.tenantId, table.branchId, table.invoiceNumber),
  studentIdx: index('idx_fee_invoices_student').on(table.studentId),
  statusIdx: index('idx_fee_invoices_status').on(table.status),
  dueIdx: index('idx_fee_invoices_due').on(table.dueDate).where(sql`status = 'pending'`),
}));
