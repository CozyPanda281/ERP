import { pgTable, uuid, varchar, text, integer, boolean, jsonb, timestamp, date, numeric, index } from 'drizzle-orm/pg-core';
...
export const tenants = pgTable('tenants', {
  ...
  slugIdx: index('idx_tenants_slug').on(table.slug).where(sql`deleted_at IS NULL`),
...
export const subscriptions = pgTable('subscriptions', {
  ...
  activeEndDateIdx: index('idx_subscriptions_end_date').on(table.endDate).where(sql`status = 'active'`),
}));
