...
export const academicYears = pgTable('academic_years', {
  ...
  tenantBranchNameUnique: uniqueIndex('academic_years_tenant_id_branch_id_name_key').on(table.tenantId, table.branchId, table.name),
  branchIdx: index('idx_academic_years_branch').on(table.branchId),
  currentIdx: index('idx_academic_years_current').on(table.branchId).where(sql`is_current = TRUE`),
}));
...
export const departments = pgTable('departments', {
  ...
  tenantBranchNameUnique: uniqueIndex('departments_tenant_id_branch_id_name_key').on(table.tenantId, table.branchId, table.name),
...
export const classes = pgTable('classes', {
  ...
  tenantBranchNameUnique: uniqueIndex('classes_tenant_id_branch_id_name_key').on(table.tenantId, table.branchId, table.name),
...
export const sections = pgTable('sections', {
  ...
  tenantBranchClassNameUnique: uniqueIndex('sections_tenant_id_branch_id_class_id_name_key').on(table.tenantId, table.branchId, table.classId, table.name),
...
export const subjects = pgTable('subjects', {
  ...
  tenantBranchCodeUnique: uniqueIndex('subjects_tenant_id_branch_id_code_key').on(table.tenantId, table.branchId, table.code),
...
export const classSubjects = pgTable('class_subjects', {
  ...
  creditHours: numeric('credit_hours', { precision: 4, scale: 1 }).default('0'),
  displayOrder: integer('display_order').default(0),
  createdAt: timestamp('created_at', { withTimezone: true }).defaultNow(),
}, (table) => ({
  tenantBranchClassSubjectUnique: uniqueIndex('class_subjects_tenant_id_branch_id_class_id_subject_id_key').on(table.tenantId, table.branchId, table.classId, table.subjectId),
...
export const teacherSubjects = pgTable('teacher_subjects', {
  ...
  uniqueAssignment: uniqueIndex('teacher_subjects_tenant_id_branch_id_teacher_id_subject_id_class_id_section_id_key').on(table.tenantId, table.branchId, table.teacherId, table.subjectId, table.classId, table.sectionId),
...
export const academicYearsRelations = relations(academicYears, ({ one }) => ({
  ...
  branch: one(branches, { fields: [academicYears.branchId], references: [branches.id] }),
}));
...
export const sectionsRelations = relations(sections, ({ one, many }) => ({
  ...
  branch: one(branches, { fields: [sections.branchId], references: [branches.id] }),
...
export const subjectsRelations = relations(subjects, ({ one, many }) => ({
  ...
  branch: one(branches, { fields: [subjects.branchId], references: [branches.id] }),
  classSubjects: many(classSubjects),
}));
