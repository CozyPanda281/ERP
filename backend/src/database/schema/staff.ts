...
export const staff = pgTable('staff', {
  ...
  employmentType: varchar('employment_type', { length: 20 }).default('permanent'),
  ...
  tenantBranchEmpCodeUnique: uniqueIndex('staff_tenant_id_branch_id_employee_code_key').on(table.tenantId, table.branchId, table.employeeCode),
  branchIdx: index('idx_staff_branch').on(table.branchId),
  departmentIdx: index('idx_staff_department').on(table.departmentId),
  teachingIdx: index('idx_staff_teaching').on(table.branchId).where(sql`is_teaching = TRUE`),
}));
...
export const staffDocumentsRelations = relations(staffDocuments, ({ one }) => ({
  tenant: one(tenants, { fields: [staffDocuments.tenantId], references: [tenants.id] }),
