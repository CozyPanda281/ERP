...
export const users = pgTable('users', {
  ...
  emailIdx: index('idx_users_email').on(table.email).where(sql`deleted_at IS NULL`),
  phoneIdx: index('idx_users_phone').on(table.phone).where(sql`deleted_at IS NULL`),
  tenantEmailUnique: uniqueIndex('idx_users_tenant_email').on(table.tenantId, table.email).where(sql`tenant_id IS NOT NULL AND deleted_at IS NULL`),
  tenantPhoneUnique: uniqueIndex('idx_users_tenant_phone').on(table.tenantId, table.phone).where(sql`tenant_id IS NOT NULL AND deleted_at IS NULL`),
}));
...
export const roles = pgTable('roles', {
  ...
  tenantSlugUnique: uniqueIndex('roles_tenant_id_slug_key').on(table.tenantId, table.slug),
...
export const userRoles = pgTable('user_roles', {
  ...
  userRoleBranchUnique: uniqueIndex('user_roles_user_id_role_id_branch_id_key').on(table.userId, table.roleId, table.branchId),
...
export const userSessions = pgTable('user_sessions', {
  ...
  activeIdx: index('idx_sessions_active').on(table.expiresAt).where(sql`is_active = TRUE`),
}));
