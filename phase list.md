...
- **2026-08-06 — v1.4 (Audit-fix batch, Phase 12 migration).** Full-project audit defects fixed and live-verified (details in `updates.md` [2026-08-06] "Full-Project Audit - Fixes"): (a) **appointments RLS gap closed** — new migration `phase12-appointments-rls.sql` (enable + `tenant_isolation` policy + FORCE; live: `t|t|1`, 0 rows as `erp_app` without tenant GUC vs 3 with) — this was the only tenant table left unprotected after phase10; (b) **`GET /students` route added** (tenant-scoped, optional `branchId`, decrypts phone/email) fixing 404s in Hostel/Library/Transport pages; list endpoints now decrypt like all other reads; (c) **uploads now served** (`useStaticAssets` `/uploads/` + nginx `/uploads/` location + `client_max_body_size 10m`; live upload→GET 200 real content); (d) **fresh-deploy bootstrap**: base schema `database/001_schema.sql` wired into compose `docker-entrypoint-initdb.d` and guarded in `migrate.sh`; healthcheck rewritten without `wget` (absent in node:22-slim); migration-loop credentials defaulted; (e) **CI**: e2e job gains JWT/encryption secrets (>=32 chars) + schema bootstrap; `security-audit` scripts exec-bit fixed; (f) hygiene: `log-audit.sh` lookbehind regex fixed + self-exclusion (was always "passing" / always "failing"); `rotate-encryption.js` covers all 15 encrypted columns + refuses implicit current-key guess (double-encryption risk) and refuses rotating to dev defaults; `database.config` defines `connectionTimeoutMillis`/`idleTimeoutMillis` (read but never defined); seed warns on missing `erp-superadmin` role row + `npm run seed`; `.env.example` drops dead `TWILIO_*`/`STORAGE_PROVIDER`, adds `SEED_ADMIN_*`; `frontend/.dockerignore`; uploads dir gitignored. Tests: **291/291 unit, 2/2 e2e** (also passing with CI-style env vars), frontend 36/36, builds clean; `docker compose config` parses; `log-audit.sh` flags phone PII and passes on a clean tree. Phase 12 migration applied to dev DB. Commit pending.
- **2026-08-06 — v1.5 (Foolproof deploy path + verified facts).** The v1.4 "fresh-deploy bootstrap" claim was WRONG and is corrected here with drill evidence: a fresh-DB drill (001 + all 8 migrations, exact alphabetical glob order used by compose initdb / `migrate.sh` / CI, `ON_ERROR_STOP=1`) found the deploy path broken at every level. Fixed, then every number below re-verified against the live reference database. Full detail in `updates.md` [2026-08-06] "Fresh-Deploy Fixes". (a) **`database/001_schema.sql` regenerated from the source of truth** (Drizzle schema → `drizzle-kit push --force` into an empty DB → `pg_dump --schema-only`): 109 tables, byte-matching the 109 `pgTable` definitions. The old hand-written file (107 CREATE TABLEs, old module names like `books`/`vehicles`/`hostel_rooms`/`student_hostel`, monthly `audit_logs` partitions, inline RLS section) had NEVER applied cleanly (`operator does not exist: text = uuid` — text GUC vs uuid column in the obsolete `superadmin_all_tenants` policy) and no longer matched the app schema; dev only worked because it was drizzle-built and 001's half-applied `ENABLE RLS` section had been partially carried over. (b) **Migrations made order-independent**: `phase10` `api_keys` policy guarded by `to_regclass`; `phase12` appointments body guarded (phase9 now carries the identical enable+policy+FORCE block); `phase7` creates `api_key_lookup` itself + FORCEs its 3 tables; `phase8` converts the 7 unique constraints to partial indexes (constraint-aware — 001 declares them as constraints, drizzle as indexes); `rls-full-tenancy` and `rls-hardening` now FORCE every table they enable (previously only tables with pre-existing policies got forced — fresh deploys ended with 9 forced tables vs 98 on dev; rls-hardening's FORCE was still behind the stale `\if :enable_force` opt-in from the pre-GUC era). (c) **Verified facts (drill + live, measured not guessed)**: fresh deploy = 001 + 8 migrations in glob order, all pass `ON_ERROR_STOP=1`; dev parity: table sets IDENTICAL (110/110, zero diffs), RLS-enforced sets IDENTICAL (98/98, zero diffs), policy-bearing tables 98/98, 0 tenant tables unprotected, seed present (erp-superadmin role, 3 plans, 17 feature flags); tests **291/291 unit + 2/2 e2e** re-run after re-applying all migrations to dev (idempotent; dev unchanged at 98 enforced). Standing fact from v1.4 (still true): audit-fix batch committed `b28028a` (pushed `c0f3ec6..b28028a`). Remaining roadmap unchanged (Phase 1 items 1.1, 1.3–1.12 etc., per Section B); this round touched only the 8 SQL files listed in `updates.md`.
